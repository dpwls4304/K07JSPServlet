...
<body>
	...
	/* 
	전송방식 get/post 에 상관없이 getParameter("파라미터명")메소드로
	...
	*/
	...
	if(!(name==null || name.equals("")) ) {
		...
		아래와같이 사용할 수 있다. 또는문자열을 연결하여 사용할 수도
		있다.
		*/
		out.println("<script>");
		...
		//out.println("</script>");
		String jsStr = "histroy.back();" + "</script>";
		out.println(jsStr);
	}
	/* 
	radio는 폼값을 아래 2가지방법을 모두 사용할 수 있다.
	...
	*/
	...
	/* 
	checkbox는 아래ㅗ아 같이 getParameterValues()를 사용한다.
	...
	*/
	...
	/* 
	textarea를 통해 입력받은 ㄱ밧은 엔터키(줄바꿈기호\r\n)가 포함되어있다.
	웹브라우저에 출력할 때는 특수기호를 <br>태그로 변환후 출력해야 한다.
	*/
	String self_intro = 
	...
	getParameterNames() : 폼값의 이름을 한꺼번에 받을 때 사용하는
		request 내장객체의 메소드로반환타입은 Enumeration형이다.
		
		hasMoreElements() : 갹채애 남은 폼값이 있는지 검사
