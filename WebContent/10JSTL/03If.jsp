...
<body>
	...
	if태그
		...
		-else문이 없기때문에 조건이 여러개일때는표현이 어렵다.
		이때는 주로 choose문을 사용한다.
		형식]
			test : 조건
			var : test의 결과를 받환받는 변수 설정
	...
	<!-- 
		...
		아래와 같이 참인조건, 거짓인조건으로 각각의 if문의 블럭을
		만들어야 한다.
	 -->
	<h3>문자열 비교</h3>
	...
	<h3>EL식으로 조건판단시 {}양쪽에 공백이 있으면 무조건
	false로 판단됨</h3>
	<c:if test=" ${true } " var="result">
	...
	<c:if test="${not empty param.user }">
		...
		<br />
		<!-- 
		첫번째 if문에서의 결과에 따라 두 번째 if문은 반대와 결과를
		가지게되는 구조로 작성되었다. 즉 loginResult가 true값으로
		설정된다면 두 번째 if문에서는 false의 조건으로 바뀌게된다. 
		 -->	
